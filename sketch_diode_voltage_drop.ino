...
void setup(void) {
  ...
  // Run through the full 12-bit scale analog output.
  for (counter = 0; counter < 4096; counter += 16 )
  {
    ...
    float Vd = Va-Vc;  // in V
    float Id = Vc/R*1000.; // in mA

    bool outputV = false; // output the diode voltage drop if true or the current if false.
    if (outputV)
      Serial.print(Vd);
    else
      Serial.print(Id);
